    ...
    >
      <div className="grid grid-cols-2 gap-3">
        ...
        <div className="aspect-square rounded-xl border border-stone-100 overflow-hidden relative">
             <img src={primaryStyle.image} alt="Brand Vibe" className="w-full h-full object-cover transition-transform duration-700 hover:scale-110" />
        </div>
...
// --- 2. Enhanced Template Skeleton (Modified Grid) ---
    ...
    return (
        <div className="w-full h-full flex flex-col bg-white overflow-hidden rounded-tl-[2rem]">
            ...
            <div className="flex-grow p-8 flex flex-col gap-8 overflow-y-auto no-scrollbar bg-slate-50/30">
                ...
                {/* Feature Grid - UPDATED LAYOUT */}
                {/* auto-rows-[11rem] ensures the grid cells have the same base height as h-44 (44 * 0.25rem = 11rem) */}
...
const ChoiceChip = ({ id, label, isSelected, onChange }) => (
    <label htmlFor={id} className={`flex items-center justify-center px-6 py-3 border rounded-full cursor-pointer transition-all duration-200 select-none ${isSelected ? 'border-black bg-gray-900 text-white shadow-md' : 'border-gray-300 bg-white text-gray-600 hover:border-gray-900 hover:text-gray-900'}`}>
  ...
  return (
    <div className="flex h-screen font-sans overflow-hidden bg-white">
      <div className="w-full lg:w-[45%] flex flex-col justify-between p-16 xl:p-20 bg-white z-30 relative shadow-[20px_0_40px_-10px_rgba(0,0,0,0.03)]">
        <div className="absolute top-10 left-10 text-3xl font-bold text-gray-900 not-italic tracking-tight">BizVistaar</div>
        <div className="flex flex-col justify-center h-full max-w-md ml-2">
            <p className="text-xs font-bold text-gray-400 mb-6 tracking-widest uppercase">STEP 2 OF 3</p>
            <h2 className="text-3xl font-bold text-gray-900 mb-6 leading-snug not-italic">Which words best describe <span className="text-gray-500">{storeName}?</span></h2>
            <p className="text-gray-400 text-sm mb-12">Select a few to help us understand your brand's personality.</p>
            <div className="flex flex-wrap gap-3 mb-8">
                {businessVibes.map(vibe => (
                    <ChoiceChip key={vibe.id} id={vibe.id} label={vibe.label} isSelected={!!selectedWords[vibe.id]} onChange={() => handleChipChange(vibe.id)} />
                ))}
            </div>
        </div>
        <div className="flex items-center justify-between w-full">
            <Link href="/get-started/1"><button className="text-gray-600 hover:text-gray-900 font-medium text-m flex items-center gap-1 transition-colors">← Back</button></Link>
            <Link href="/get-started/3" passHref className={!hasSelection ? "pointer-events-none" : ""}><button onClick={handleContinue} disabled={!hasSelection} className="px-6 py-2.5 bg-gray-900 text-white font-medium text-sm rounded-full hover:bg-gray-700 disabled:bg-gray-300 disabled:text-gray-500 transition-colors shadow-sm">Continue</button></Link>
        </div>
      </div>
      <div className="hidden lg:block lg:w-[55%] bg-gray-50 relative overflow-hidden border-l border-gray-100">
        <div className="absolute inset-0 z-0 pointer-events-none opacity-30"><GridBackgroundDemo /></div>
        <div className="relative w-full h-full">
             <MockBrowser storeName={storeName} selectedVibes={selectedWords} activeVibe={activeVibe} className="absolute top-[10%] left-[35%] w-[100%] h-[100%] z-20" />
        </div>
      </div>
    </div>
  );
