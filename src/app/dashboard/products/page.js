...
export default function ProductsPage() {
  ...
  const [stockFilters, setStockFilters] = useState([]); // ['Low Stock', 'Out Of Stock', ...]
  ...
  const fetchBaseData = useCallback(async () => {
    setLoading(true);
    try {
      // If categories are empty, fetch them once.
      if (categories.length === 0) {
        const cats = await getCategories();
        setCategories(cats || []);
      }

      // Pass all filter params to server action
  ...
  }, [searchTerm, currentPage, selectedCategory, stockFilters, categories.length]);
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">Manage your product catalog and view sales performance.</p>
        </div>
